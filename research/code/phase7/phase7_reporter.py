...
def load_mode_series(mode_dir: Path) -> dict[str, list[tuple[float, float]]]:
    ...
    burst_fill_allow: list[tuple[int, float]] = []
    ...
    for log_path in sorted(chosen_worker.glob("nccl.*.log")):
        for line in log_path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if event == "RATE_ALLOW":
                t_ns = int(row7.get("tNs", "0"))
                allow_series.append((t_ns, float(row7.get("postCost", "0"))))
                tokens_before = float(row7.get("tokensBefore", "0"))
                target_burst = float(row7.get("targetBurst", "0"))
                if target_burst > 0:
                    burst_fill_allow.append((t_ns, tokens_before / target_burst))
    ...
    return {
        ...
        "burst_fill_allow": [((t - t0) / 1e6, y) for t, y in sorted(burst_fill_allow) if t > 0],
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render phase7 diagnostic plots")
    ...
    for repeat_dir in repeat_dirs:
        ...
        burst_fill_plot = output_dir / f"{repeat_dir.name}_burst_fill_ratio_overlay.png"
        ...
        plot_cumulative_overlay(
            {mode: values["burst_fill_allow"] for mode, values in cumulative_data.items()},
            title=f"{repeat_dir.name} allow-time burst fill ratio (worker01)",
            ylabel="tokensBefore / targetBurst",
            output_path=burst_fill_plot,
        )
        ...
        diag_summary_path = repeat_dir / "phase7_diag_summary.json"
        diag_by_mode = {}
        if diag_summary_path.exists():
            diag_payload = json.loads(diag_summary_path.read_text(encoding="utf-8"))
            diag_by_mode = {row["mode"]: row for row in diag_payload.get("modes", [])}
        ...
        for mode_dir in mode_dirs:
            ...
            diag_row = diag_by_mode.get(mode_dir.name, {})
            summary_rows.append(
                {
                    "mode": mode_dir.name,
                    ...
                    "baseline_rate_per_ms_avg": diag_row.get("baseline_rate_per_ms", {}).get("avg", 0),
                    "target_rate_per_ms_avg": diag_row.get("target_rate_per_ms", {}).get("avg", 0),
                    "target_burst_posts_avg": diag_row.get("target_burst_posts", {}).get("avg", 0),
                    "burst_fill_ratio_allow_avg": diag_row.get("burst_fill_ratio_allow", {}).get("avg", 0),
                    "burst_fill_ratio_stall_avg": diag_row.get("burst_fill_ratio_stall", {}).get("avg", 0),
                }
            )
        (output_dir / f"{repeat_dir.name}_phase7_report_summary.json").write_text(
        ...
        html_sections.append(
            ...
            <section>
              ...
              <img src="{burst_fill_plot.name}" style="max-width: 100%;"><br>
    ...
    html = f"""<!doctype html>
<html><head><meta charset="utf-8"><title>Phase7 Report</title></head>
<body>
  <h1>Phase7 Runtime-relative Post-rate Report</h1>
  <p>Relative-time overlays for cumulative recv POST, runtime-derived rate limiter events, latency, and throughput.</p>
  {''.join(html_sections)}
</body></html>"""
    (output_dir / "phase7_report.html").write_text(html, encoding="utf-8")
