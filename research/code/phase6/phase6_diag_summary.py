...
def analyze_mode(mode_dir: Path) -> dict:
    ...
    for log_path in sorted(mode_dir.glob("worker*/nccl.*.log")):
        for line in log_path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if event == "RATE_ALLOW":
                allow_times_ns.append(int(row6.get("tNs", "0")))
                allow_costs.append(float(row6.get("postCost", "0")))
                tokens_after_allow.append(float(row6.get("tokensAfter", "0")))
            elif event == "RATE_STALL":
                stall_times_ns.append(int(row6.get("tNs", "0")))
                tokens_after_stall.append(float(row6.get("tokensAfter", "0")))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize phase6 post-rate diagnostics")
    ...
    print(f"[phase6-summary] experiment_root={experiment_root}")
    for mode in summary["modes"]:
        print(
            f"[phase6-summary] mode={mode['mode']} "
    ...
    print(f"[phase6-summary] wrote {output_json}")
