...
PHASE6_ENABLE_VALUE=${PHASE6_ENABLE_VALUE:-}
PHASE6_POST_RATE_VALUE=${PHASE6_POST_RATE_VALUE:-}
PHASE6_POST_BURST_VALUE=${PHASE6_POST_BURST_VALUE:-}
...
export NCCL_PHASE6_LOG=${NCCL_PHASE6_LOG:-1}
...
case "${MODE}" in
  stock)
    ...
    ;;
  p6_*)
    export NCCL_PHASE6_ENABLE=${PHASE6_ENABLE_VALUE:-1}
    export NCCL_PHASE6_POST_RATE=${PHASE6_POST_RATE_VALUE:?PHASE6_POST_RATE_VALUE is required for ${MODE}}
    export NCCL_PHASE6_POST_BURST=${PHASE6_POST_BURST_VALUE:?PHASE6_POST_BURST_VALUE is required for ${MODE}}
    ;;
  *)
    echo "[phase6-worker] unsupported MODE=${MODE}" >&2
    exit 1
    ;;
esac
...
echo "[phase6-worker] RUN_ID=${RUN_ID} EXPERIMENT=${EXPERIMENT_LABEL}"
echo "[phase6-worker] REPEAT_LABEL=${REPEAT_LABEL}"
echo "[phase6-worker] WORKER_NAME=${WORKER_NAME} NODE_RANK=${NODE_RANK}/${NNODES} MODE=${MODE_UPPER}"
echo "[phase6-worker] MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MASTER_PORT}"
echo "[phase6-worker] PHASE6_ENABLE=${NCCL_PHASE6_ENABLE} PHASE6_POST_RATE=${NCCL_PHASE6_POST_RATE} PHASE6_POST_BURST=${NCCL_PHASE6_POST_BURST}"
echo "[phase6-worker] HIDDEN_DIM=${HIDDEN_DIM} NUM_LAYERS=${NUM_LAYERS} BATCH_SIZE=${BATCH_SIZE} BUCKET_CAP_MB=${BUCKET_CAP_MB} LR=${LR}"
echo "[phase6-worker] MODEL_SEED=${MODEL_SEED}"
echo "[phase6-worker] NCCL_ALGO=${ALGO_SETTING} NCCL_PROTO=${PROTO_SETTING} STATUS_FILE=${STATUS_FILE}"
echo "[phase6-worker] NCCL_DEBUG_FILE=${NCCL_DEBUG_FILE}"
...
if (( RUN_RC == 0 )); then
  mark_success
  echo "[phase6-worker] complete worker=${WORKER_NAME} mode=${MODE_UPPER}"
  exit 0
fi
...
echo "[phase6-worker] failure worker=${WORKER_NAME} mode=${MODE_UPPER} rc=${RUN_RC} message=${STATUS_MESSAGE}" >&2
exit "${RUN_RC}"
