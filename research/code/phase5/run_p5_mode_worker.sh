...
export NCCL_PHASE5_PROGRESS_LOG_EVERY=${NCCL_PHASE5_PROGRESS_LOG_EVERY:-64}
...
case "${MODE}" in
  ...
  p5_w*)
    ...
    export NCCL_PHASE4_POST_RECEIVE_W=${PHASE4_POST_RECEIVE_W_VALUE:-${MODE#p5_w}}
    ;;
  *)
    echo "[phase5-worker] unsupported MODE=${MODE}" >&2
    exit 1
    ;;
esac
...
echo "[phase5-worker] RUN_ID=${RUN_ID} EXPERIMENT=${EXPERIMENT_LABEL}"
echo "[phase5-worker] REPEAT_LABEL=${REPEAT_LABEL}"
echo "[phase5-worker] WORKER_NAME=${WORKER_NAME} NODE_RANK=${NODE_RANK}/${NNODES} MODE=${MODE_UPPER}"
echo "[phase5-worker] MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MASTER_PORT}"
echo "[phase5-worker] PHASE4_ENABLE=${NCCL_PHASE4_ENABLE} PHASE4_POST_RECEIVE_W=${NCCL_PHASE4_POST_RECEIVE_W}"
echo "[phase5-worker] PHASE5_LOG=${NCCL_PHASE5_LOG} PHASE5_PROGRESS_LOG_EVERY=${NCCL_PHASE5_PROGRESS_LOG_EVERY}"
echo "[phase5-worker] HIDDEN_DIM=${HIDDEN_DIM} NUM_LAYERS=${NUM_LAYERS} BATCH_SIZE=${BATCH_SIZE} BUCKET_CAP_MB=${BUCKET_CAP_MB} LR=${LR}"
echo "[phase5-worker] MODEL_SEED=${MODEL_SEED}"
echo "[phase5-worker] NCCL_ALGO=${ALGO_SETTING} NCCL_PROTO=${PROTO_SETTING} STATUS_FILE=${STATUS_FILE}"
echo "[phase5-worker] NCCL_DEBUG_FILE=${NCCL_DEBUG_FILE}"
...
if (( RUN_RC == 0 )); then
  mark_success
  echo "[phase5-worker] complete worker=${WORKER_NAME} mode=${MODE_UPPER}"
  exit 0
fi
...
echo "[phase5-worker] failure worker=${WORKER_NAME} mode=${MODE_UPPER} rc=${RUN_RC} message=${STATUS_MESSAGE}" >&2
exit "${RUN_RC}"
