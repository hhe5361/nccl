...
RUN_ID=${RUN_ID:-phase3_b3_mixed8_allreduce_$(date +%y%m%d_%H%M%S)}
...
ALLREDUCE_ALGOS=${ALLREDUCE_ALGOS:-Ring,Tree}
...
require_sshpass() {
  if ! command -v sshpass >/dev/null 2>&1; then
    echo "[phase3-mixed8-allreduce] sshpass is required for switch logger integration." >&2
    exit 1
  fi
}
...
remote_dpu_bash() {
  ...
  if [[ -z "${DPU_NODE_PWD:-}" ]]; then
    echo "[phase3-mixed8-allreduce] DPU_NODE_PWD must be set when SWITCH_LOG_ENABLE=1 on MASTER_SERVER." >&2
    exit 1
  fi
...
wait_for_switch_metadata() {
  ...
  while [[ ! -f "${SWITCH_METADATA_FILE}" ]]; do
    if (( waited >= timeout_sec )); then
      echo "[phase3-mixed8-allreduce] timed out waiting for switch metadata file ${SWITCH_METADATA_FILE}" >&2
      exit 1
    fi
    sleep 2
    waited=$((waited + 2))
...
start_switch_logger() {
  ...
  if [[ -z "${SWITCH_LOG_RUN_ID}" || -z "${SWITCH_LOG_PID_FILE}" ]]; then
    echo "[phase3-mixed8-allreduce] failed to parse switch logger metadata" >&2
    echo "${output}" >&2
    exit 1
  fi
  ...
  echo "[phase3-mixed8-allreduce] switch logger started run_id=${SWITCH_LOG_RUN_ID} log_dir=${SWITCH_LOG_DIR} local_dir=${SWITCH_LOG_LOCAL_DIR}"
}
...
stop_switch_logger() {
  ...
  echo "[phase3-mixed8-allreduce] switch logger stopped run_id=${SWITCH_LOG_RUN_ID}"
}
...
IFS=',' read -r -a _algo_items <<< "${ALLREDUCE_ALGOS}"
for algo in "${_algo_items[@]}"; do
  [[ -n "${algo}" ]] || continue
  algo_lc=$(echo "${algo}" | tr '[:upper:]' '[:lower:]')
  add_experiment "mixed8_allreduce_${algo_lc}_${PAYLOAD_MB}mb" "${MIXED_8_HOSTS}" "mixed-8" "allreduce" "${algo}"
done
...
echo "[phase3-mixed8-allreduce] RUN_ID=${RUN_ID}"
echo "[phase3-mixed8-allreduce] WORKER_NAME=${WORKER_NAME}"
echo "[phase3-mixed8-allreduce] MASTER_SERVER=${MASTER_SERVER}"
echo "[phase3-mixed8-allreduce] MATRIX_ROOT=${MATRIX_ROOT}"
echo "[phase3-mixed8-allreduce] TOTAL_EXPERIMENTS=${#EXPERIMENT_IDS[@]}"
echo "[phase3-mixed8-allreduce] ALLREDUCE_ALGOS=${ALLREDUCE_ALGOS}"
if [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
  echo "[phase3-mixed8-allreduce] SWITCH_METADATA_FILE=${SWITCH_METADATA_FILE}"
fi
...
for idx in "${!EXPERIMENT_IDS[@]}"; do
  ...
  if contains_worker "${WORKER_NAME}" "${exp_hosts}"; then
    ...
    echo "[phase3-mixed8-allreduce] start idx=${idx} id=${exp_id} rank=${exp_rank}/${exp_nodes} placement=${exp_placement} coll=${exp_coll} algo=${exp_algo} payload=${PAYLOAD_MB}MB"
    set +e
    RUN_ID="${exp_id}" \
    LOG_ROOT="${exp_root}" \
    ...
    echo "[phase3-mixed8-allreduce] skip idx=${idx} id=${exp_id} worker=${WORKER_NAME}"
  fi
  ...
  if (( failed == 1 )); then
    echo "[phase3-mixed8-allreduce] experiment failed idx=${idx} id=${exp_id}" >&2
    exit 1
  fi

  echo "[phase3-mixed8-allreduce] complete idx=${idx} id=${exp_id}"
done

echo "[phase3-mixed8-allreduce] all experiments completed"
