...
case "${MODE}" in
  ...
  b3)
    ...
    ;;
  *)
    echo "[phase3-worker] unsupported MODE=${MODE}" >&2
    exit 1
    ;;
esac
...
then
  ...
  echo "[phase3-worker] PyTorch launcher not found in current environment." >&2
  exit 1
fi
...
echo "[phase3-worker] RUN_ID=${RUN_ID} EXPERIMENT=${EXPERIMENT_LABEL}"
echo "[phase3-worker] WORKER_NAME=${WORKER_NAME} NODE_RANK=${NODE_RANK}/${NNODES} MODE=${MODE_UPPER}"
echo "[phase3-worker] MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MASTER_PORT}"
echo "[phase3-worker] COLLECTIVE=${COLLECTIVE} PAYLOAD_MB=${PAYLOAD_MB} NCCL_ALGO=${ALGO_SETTING} NCCL_PROTO=${PROTO_SETTING}"
echo "[phase3-worker] B3_ENABLE=${NCCL_PHASE3_B3_ENABLE} STATUS_FILE=${STATUS_FILE}"
echo "[phase3-worker] NCCL_DEBUG_FILE=${NCCL_DEBUG_FILE}"
...
if (( RUN_RC == 0 )); then
  mark_success
  echo "[phase3-worker] complete worker=${WORKER_NAME} mode=${MODE_UPPER}"
  exit 0
fi
...
echo "[phase3-worker] failure worker=${WORKER_NAME} mode=${MODE_UPPER} rc=${RUN_RC} message=${STATUS_MESSAGE}" >&2
exit "${RUN_RC}"
