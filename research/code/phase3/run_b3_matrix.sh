...
require_sshpass() {
  if ! command -v sshpass >/dev/null 2>&1; then
    echo "[phase3-matrix] sshpass is required for switch logger integration." >&2
    exit 1
  fi
}
...
remote_dpu_bash() {
  ...
  if [[ -z "${DPU_NODE_PWD:-}" ]]; then
    echo "[phase3-matrix] DPU_NODE_PWD must be set when SWITCH_LOG_ENABLE=1 on MASTER_SERVER." >&2
    exit 1
  fi
...
wait_for_switch_metadata() {
  ...
  while [[ ! -f "${SWITCH_METADATA_FILE}" ]]; do
    if (( waited >= timeout_sec )); then
      echo "[phase3-matrix] timed out waiting for switch metadata file ${SWITCH_METADATA_FILE}" >&2
      exit 1
    fi
    sleep 2
    waited=$((waited + 2))
...
start_switch_logger() {
  ...
  echo "[phase3-matrix] switch logger started run_id=${SWITCH_LOG_RUN_ID} log_dir=${SWITCH_LOG_DIR} local_dir=${SWITCH_LOG_LOCAL_DIR}"
}
...
cleanup() {
  if (( SWITCH_LOG_STARTED == 1 )) && is_master_server; then
    emit_switch_marker "matrix_end" "run_id=${RUN_ID}" "phase3_matrix"
    stop_switch_logger
  fi
}
trap cleanup EXIT
...
echo "[phase3-matrix] WORKER_NAME=${WORKER_NAME}"
echo "[phase3-matrix] MASTER_SERVER=${MASTER_SERVER}"
echo "[phase3-matrix] MATRIX_ROOT=${MATRIX_ROOT}"
echo "[phase3-matrix] TOTAL_EXPERIMENTS=${#EXPERIMENT_IDS[@]}"
if [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
  echo "[phase3-matrix] SWITCH_METADATA_FILE=${SWITCH_METADATA_FILE}"
fi
...
for idx in "${!EXPERIMENT_IDS[@]}"; do
  ...
  if is_master_server && [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
    emit_switch_marker "exp_start" "experiment=${exp_id} placement=${exp_placement} collective=${exp_coll} algo=${exp_algo} payload_mb=${PAYLOAD_MB}" "phase3_matrix"
  fi
  ...
  if contains_worker "${WORKER_NAME}" "${exp_hosts}"; then
    ...
    echo "[phase3-matrix] start idx=${idx} id=${exp_id} rank=${exp_rank}/${exp_nodes} placement=${exp_placement} coll=${exp_coll} algo=${exp_algo} payload=${PAYLOAD_MB}MB"
    set +e
    RUN_ID="${exp_id}"     LOG_ROOT="${exp_root}"     MASTER_ADDR="${MASTER_ADDR}"     MASTER_PORT_BASE="${port_base}"     NNODES="${exp_nodes}"     NPROC_PER_NODE=1     NODE_RANK="${exp_rank}"     WORKER_NAME="${WORKER_NAME}"     MASTER_SERVER="${MASTER_SERVER}"     SWITCH_LOG_ENABLE="${SWITCH_LOG_ENABLE}"     SWITCH_METADATA_FILE="${SWITCH_METADATA_FILE}"     DPU_NODE_HOST="${DPU_NODE_HOST}"     DPU_NODE_USER="${DPU_NODE_USER}"     SWITCH_LOGGER_ROOT="${SWITCH_LOGGER_ROOT}"     SWITCH_LOG_SHARED_ROOT="${SWITCH_LOG_SHARED_ROOT}"     SWITCH_LOG_LOCAL_DIR="${SWITCH_LOG_LOCAL_DIR}"     PLACEMENT="${exp_placement}"     COLLECTIVE="${exp_coll}"     RUN_MODES="${RUN_MODES}"     PAYLOAD_MB="${PAYLOAD_MB}"     DTYPE="${DTYPE}"     STEPS="${STEPS}"     WARMUP_STEPS="${WARMUP_STEPS}"     SLEEP_MS="${SLEEP_MS}"     NCCL_ALGO="${exp_algo}"     NCCL_PROTO="auto"     NCCL_RACK_MAP_FILE="${RACK_MAP_FILE}"     TORCH_ENV="${TORCH_ENV}"     bash "${REPO_ROOT}/${INNER_SCRIPT}"
    run_rc=$?
    set -e
  else
    echo "[phase3-matrix] skip idx=${idx} id=${exp_id} worker=${WORKER_NAME}"
  fi
  ...
  if is_master_server && [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
    emit_switch_marker "exp_end" "experiment=${exp_id} placement=${exp_placement} collective=${exp_coll} algo=${exp_algo} payload_mb=${PAYLOAD_MB} rc=${failed}" "phase3_matrix"
  fi

  if (( failed == 1 )); then
    echo "[phase3-matrix] experiment failed idx=${idx} id=${exp_id}" >&2
    exit 1
  fi

  echo "[phase3-matrix] complete idx=${idx} id=${exp_id}"
done

echo "[phase3-matrix] all experiments completed"
