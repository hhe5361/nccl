...
# - 컨테이너가 멈춰 있으면 start
# - 기본은 docker exec 로 shell/command 진입
# - --start-only 로 준비만 하고 종료 가능
# - --reset 으로 clean start 가능
...
if [[ ${#CMD[@]} -eq 0 && ${START_ONLY} -eq 0 ]]; then
  CMD=(bash)
fi
...
if [[ ${RESET_CONTAINER} -eq 1 ]] && container_exists; then
  echo "[docker] removing existing container ${CONTAINER_NAME} for clean start"
