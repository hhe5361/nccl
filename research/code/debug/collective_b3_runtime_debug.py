...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 3 B3 collective workload with runtime debug events")
    parser.add_argument(
        "--collective",
...
class DebugLogger:
    def __init__(self, path: Optional[Path], rank: int, local_rank: int, worker: str) -> None:
        self.path = path
        self.rank = rank
...
def main() -> None:
    ...
    worker_name = os.environ.get("WORKER_NAME", f"worker_local{local_rank}")
    ...
    try:
        ...
        debug_logger.log(
            ...
            nccl_debug_file=os.environ.get("NCCL_DEBUG_FILE", ""),
        ...
        if rank == 0:
            print(
                f"phase3_b3_debug collective={args.collective} steps={args.steps} warmup_steps={args.warmup_steps} "
        ...
        records = []
        debug_logger.log("pre_start_barrier_enter")
        dist.barrier()
        debug_logger.log("pre_start_barrier_exit")
        ...
        try:
            for step in range(args.steps):
                debug_logger.log("step_enter", step=step, warmup=step < args.warmup_steps)
                ...
                debug_logger.log("step_collective_exit", step=step, local_step_ms=local_ms)
                ...
                debug_logger.log("step_metrics_reduce_exit", step=step)
                ...
                if abs(sample - expected) > 1e-2:
                    debug_logger.log("validation_failed", step=step, sample=sample, expected=expected)
                    raise RuntimeError(
                ...
                debug_logger.log("step_complete", step=step, local_step_ms=local_ms)
        ...
        if rank == 0 and output_dir is not None:
            debug_logger.log("summary_write_enter")
            ...
            debug_logger.log("summary_write_exit", summary_path=str(summary_path))

        debug_logger.log("final_barrier_enter")
        dist.barrier()
        debug_logger.log("final_barrier_exit")
        debug_logger.log("destroy_pg_enter")
        dist.destroy_process_group()
        debug_logger.log("destroy_pg_exit")
