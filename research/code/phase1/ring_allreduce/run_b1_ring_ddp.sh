#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd "${SCRIPT_DIR}/../../.." && pwd)

infer_node_rank() {
  local worker_name=$1
  if [[ "${worker_name}" =~ ^worker([0-9]+)$ ]]; then
    local idx=${BASH_REMATCH[1]}
    if (( 1 <= 10#${idx} && 10#${idx} <= 7 )); then
      echo $((10#${idx} - 1))
      return 0
    fi
  fi
  echo "cannot infer NODE_RANK from WORKER_NAME='${worker_name}'" >&2
  return 1
}

validate_w_list() {
  local raw_list=$1
  local value
  IFS=',' read -r -a parsed <<< "${raw_list}"
  if (( ${#parsed[@]} == 0 )); then
    echo "[phase1] W_LIST is empty." >&2
    return 1
  fi
  for value in "${parsed[@]}"; do
    if [[ ! "${value}" =~ ^[0-9]+$ ]]; then
      echo "[phase1] W_LIST contains non-integer entry '${value}'." >&2
      return 1
    fi
    if (( 10#${value} <= 0 )); then
      echo "[phase1] W_LIST entry '${value}' must be > 0." >&2
      return 1
    fi
  done
}

RUN_ID=${RUN_ID:-phase1_b1_ring_manual}
MASTER_ADDR=${MASTER_ADDR:-172.16.0.101}
MASTER_PORT_BASE=${MASTER_PORT_BASE:-29500}
NNODES=${NNODES:-7}
NPROC_PER_NODE=${NPROC_PER_NODE:-1}
WORKER_NAME=${WORKER_NAME:-$(hostname -s)}
NODE_RANK=${NODE_RANK:-$(infer_node_rank "${WORKER_NAME}")}
TORCH_ENV=${TORCH_ENV:-/workspace/venvs/torch-cu121-custom/bin/activate}
TARGET_SCRIPT=${TARGET_SCRIPT:-research/code/phase1/ring_allreduce/ddp_ring_b1.py}
LOG_ROOT=${LOG_ROOT:-/mnt/nfs_share/cts_experiments/${RUN_ID}}
W_LIST=${W_LIST:-1,2,4,5,6,7,8}
STEPS=${STEPS:-40}
WARMUP_STEPS=${WARMUP_STEPS:-5}
PARAM_MB=${PARAM_MB:-64}
DTYPE=${DTYPE:-float32}
BUCKET_CAP_MB=${BUCKET_CAP_MB:-128}
SLEEP_MS=${SLEEP_MS:-0}

mkdir -p "${LOG_ROOT}"

# shellcheck disable=SC1090
source "${TORCH_ENV}"

export LD_PRELOAD="/workspace/nccl/build/lib/libnccl.so${LD_PRELOAD:+:${LD_PRELOAD}}"
export NCCL_PHASE0_LOG=${NCCL_PHASE0_LOG:-1}
export NCCL_DEBUG=${NCCL_DEBUG:-INFO}
export NCCL_DEBUG_SUBSYS=${NCCL_DEBUG_SUBSYS:-NET}
export NCCL_ALGO=${NCCL_ALGO:-Ring}
export NCCL_PROTO=${NCCL_PROTO:-Simple}
export NCCL_NET_GDR_LEVEL=${NCCL_NET_GDR_LEVEL:-0}
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
export OMP_NUM_THREADS=${OMP_NUM_THREADS:-1}

if command -v torchrun >/dev/null 2>&1; then
  LAUNCHER=(torchrun)
elif python - <<'PYTORCHCHECK' >/dev/null 2>&1
import importlib.util
import sys
sys.exit(0 if importlib.util.find_spec('torch.distributed.run') else 1)
PYTORCHCHECK
then
  LAUNCHER=(python -m torch.distributed.run)
else
  echo "[phase1] PyTorch launcher not found in current environment." >&2
  exit 1
fi

IFS=',' read -r -a W_VALUES <<< "${W_LIST}"
validate_w_list "${W_LIST}"

echo "[phase1] RUN_ID=${RUN_ID}"
echo "[phase1] WORKER_NAME=${WORKER_NAME} NODE_RANK=${NODE_RANK}/${NNODES}"
echo "[phase1] MASTER_ADDR=${MASTER_ADDR} MASTER_PORT_BASE=${MASTER_PORT_BASE}"
echo "[phase1] LOG_ROOT=${LOG_ROOT}"
echo "[phase1] TARGET_SCRIPT=${TARGET_SCRIPT}"
echo "[phase1] W_LIST=${W_LIST}"
echo "[phase1] PARAM_MB=${PARAM_MB} DTYPE=${DTYPE} BUCKET_CAP_MB=${BUCKET_CAP_MB}"

for idx in "${!W_VALUES[@]}"; do
  W="${W_VALUES[$idx]}"
  RUN_ROOT="${LOG_ROOT}/W${W}"
  WORKER_LOG_ROOT="${RUN_ROOT}/${WORKER_NAME}"
  MASTER_PORT=$((MASTER_PORT_BASE + idx))

  mkdir -p "${RUN_ROOT}" "${WORKER_LOG_ROOT}"

  export NCCL_PHASE1_STATIC_W="${W}"
  export NCCL_DEBUG_FILE="${WORKER_LOG_ROOT}/nccl.%h.%p.log"
  export PHASE1_OUTPUT_DIR="${RUN_ROOT}"

  echo "[phase1] starting W=${W} MASTER_PORT=${MASTER_PORT}"
  echo "[phase1] NCCL_DEBUG_FILE=${NCCL_DEBUG_FILE}"

  "${LAUNCHER[@]}" \
    --nnodes="${NNODES}" \
    --nproc_per_node="${NPROC_PER_NODE}" \
    --node_rank="${NODE_RANK}" \
    --master_addr="${MASTER_ADDR}" \
    --master_port="${MASTER_PORT}" \
    "${REPO_ROOT}/${TARGET_SCRIPT}" \
    --steps "${STEPS}" \
    --warmup-steps "${WARMUP_STEPS}" \
    --param-mb "${PARAM_MB}" \
    --dtype "${DTYPE}" \
    --bucket-cap-mb "${BUCKET_CAP_MB}" \
    --sleep-ms "${SLEEP_MS}" \
    --output-dir "${RUN_ROOT}" \
    --tag "W${W}" \
    "$@"

  sleep 2
done
