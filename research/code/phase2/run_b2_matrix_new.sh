...
require_sshpass() {
  if ! command -v sshpass >/dev/null 2>&1; then
    echo "[phase2-matrix-new] sshpass is required for switch logger integration." >&2
    exit 1
  fi
}
...
remote_dpu_bash() {
  ...
  if [[ -z "${DPU_NODE_PWD:-}" ]]; then
    echo "[phase2-matrix-new] DPU_NODE_PWD must be set when SWITCH_LOG_ENABLE=1 on MASTER_SERVER." >&2
    exit 1
  fi
...
wait_for_switch_metadata() {
  ...
  while [[ ! -f "${SWITCH_METADATA_FILE}" ]]; do
    if (( waited >= timeout_sec )); then
      echo "[phase2-matrix-new] timed out waiting for switch metadata file ${SWITCH_METADATA_FILE}" >&2
      exit 1
    fi
    sleep 2
    waited=$((waited + 2))
...
start_switch_logger() {
  ...
  if [[ -z "${SWITCH_LOG_RUN_ID}" || -z "${SWITCH_LOG_PID_FILE}" ]]; then
    echo "[phase2-matrix-new] failed to parse switch logger metadata" >&2
    echo "${output}" >&2
    exit 1
  fi
  ...
  echo "[phase2-matrix-new] switch logger started run_id=${SWITCH_LOG_RUN_ID} log_dir=${SWITCH_LOG_DIR} local_dir=${SWITCH_LOG_LOCAL_DIR}"
}
...
stop_switch_logger() {
  ...
  echo "[phase2-matrix-new] switch logger stopped run_id=${SWITCH_LOG_RUN_ID}"
}
...
echo "[phase2-matrix-new] RUN_ID=${RUN_ID}"
echo "[phase2-matrix-new] WORKER_NAME=${WORKER_NAME}"
echo "[phase2-matrix-new] MASTER_SERVER=${MASTER_SERVER}"
echo "[phase2-matrix-new] MATRIX_ROOT=${MATRIX_ROOT}"
echo "[phase2-matrix-new] TOTAL_EXPERIMENTS=${#EXPERIMENT_IDS[@]}"
if [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
  echo "[phase2-matrix-new] SWITCH_METADATA_FILE=${SWITCH_METADATA_FILE}"
fi
...
for idx in "${!EXPERIMENT_IDS[@]}"; do
  ...
  if is_master_server && [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
    emit_switch_marker_best_effort "exp_start" "experiment=${exp_id} collective=${exp_coll} payload_mb=${exp_payload}" "phase2_matrix"
  fi
  ...
  if contains_worker "${WORKER_NAME}" "${exp_hosts}"; then
    ...
    echo "[phase2-matrix-new] start idx=${idx} id=${exp_id} rank=${exp_rank}/${exp_nodes} coll=${exp_coll} payload=${exp_payload}MB"
    set +e
    RUN_ID="${exp_id}" \
    LOG_ROOT="${exp_root}" \
    ...
    echo "[phase2-matrix-new] skip idx=${idx} id=${exp_id} worker=${WORKER_NAME}"
  fi
  ...
  if is_master_server && [[ "${SWITCH_LOG_ENABLE}" == "1" ]]; then
    emit_switch_marker_best_effort "exp_end" "experiment=${exp_id} collective=${exp_coll} payload_mb=${exp_payload} rc=${failed}" "phase2_matrix"
  fi

  if (( failed == 1 )); then
    echo "[phase2-matrix-new] experiment failed idx=${idx} id=${exp_id}" >&2
    exit 1
  fi

  echo "[phase2-matrix-new] complete idx=${idx} id=${exp_id}"
done

echo "[phase2-matrix-new] all experiments completed"
