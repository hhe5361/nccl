...
def main():
    parser = argparse.ArgumentParser(description="Compare B2 final output digests against STOCK")
    ...
    print(f"[phase2-compare] experiment_root={experiment_root}")
    for mode in summary["modes"]:
        print(
            f"[phase2-compare] mode={mode['mode']} "
            f"matched={mode['workers_matched']}/{mode['workers_total']} "
            f"all_matched={mode['all_matched']}"
        )
    print(f"[phase2-compare] wrote {output_json}")
